...
struct SingleLineLollipop: View {
    ...
    var body: some View {
        if isOverview {
            ...
            List {
                ...
                Text("Here is the Sleeping __Time analisis__")
                    .font(.system(size: 32))
  ...
  private func getBaselineMarker (marker: Sale) -> some ChartContent {
      return LineMark(
          ...
          y: .value("Sales", marker.hoursOfSleep)
      )
      .accessibilityLabel(marker.day.formatted(date: .complete, time: .omitted))
      .accessibilityValue("\(marker.hoursOfSleep) sold")
                    ...
                    if showLollipop,
                       let selectedElement {
                        ...
                        VStack(alignment: .center) {
                            ...
                            Text("\(selectedElement.hoursOfSleep, format: .number)")
                                .font(.title2.bold())
